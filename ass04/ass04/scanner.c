...
*	param: char lexeme[]
*	version: 2.1 UPDATED FOR PARSER
*	author: Seyedamin Seyedmahmoudian
*	return: Token
*/
...
Token aa_func05(char lexeme[]){
	...
	if ((intValue >= SHRT_MIN && intValue <= SHRT_MAX) || intValue == ZERO) {
		t.code = INL_T;
...
Token aa_func08(char lexeme[]){
	...
	if ((floatVal >= FLT_MIN && floatVal <= FLT_MAX) || floatVal == ZERO){
		t.code = FPL_T;
...
Token aa_func11(char lexeme[]){
	...
	if ((longVal >= SHRT_MIN && longVal <= SHRT_MAX) || longVal == ZERO){
		t.code = INL_T;
