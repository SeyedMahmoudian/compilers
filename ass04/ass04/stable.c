...
int st_install(STD sym_table, char* lexeme, char type, int line){
	...
	if (flag){
		...
		for (i = ZERO; i <=(unsigned int) sym_table.st_offset; i++){
			sym_table.pstvr[i].plex = size + b_cbhead(sym_table.plsBD);
			/*sym_table.pstvr[i].plex = b_setmark(sym_table.plsBD,(short)size);*/
			size += (short)strlen(sym_table.pstvr[i].plex) + 1;
